...
def CalcOfDistancesPolar2D(num_pairs):
    ...
    for i, pair in enumerate(pairs, start=1):
        #print(f"{i}: Point 1: r: {pair[0][0]:.2f}, theta:  {pair[0][1]:.2f} rad ({round(np.degrees(pair[0][1]), 2)}°)   "
        #      f"|   Point 2: r: {pair[1][0]:.2f}, theta:   {pair[1][1]:.2f}rad ({round(np.degrees(pair[1][1]), 2)}°)")
        d = round(np.sqrt(
            (pair[0][0] ** 2 + pair[1][0] ** 2 - 2 * pair[0][0] * pair[1][0] * np.cos(pair[1][1] - pair[0][1]))), 2)
        print(f"{i} Distance between points: {d}")
...
def CalcOfDistancesSphericalThroughTheSphereVolume(num_pairs):
    ...
    for i, pair in enumerate(pairs, start=1):
        #print(f"{i}: Point 1: r: {pair[0][0]:.2f}, theta:  {pair[0][1]:.2f} rad ({round(np.degrees(pair[0][1]), 2)}°), phi(Latitude): {pair[0][2]:.2f} rad ({round(np.degrees(pair[0][2]), 2)}°)  "
        #      f"|   Point 2: r: {pair[1][0]:.2f}, theta:   {pair[1][1]:.2f} rad ({round(np.degrees(pair[1][1]), 2)}°), phi(Latitude): {pair[1][2]:.2f} rad ({round(np.degrees(pair[1][2]), 2)}°)")
...
def CalcOfDistancesSphericalOnTheSphereSurface(num_pairs):
    ...
    for i, pair in enumerate(pairs, start=1):
        #print(f"{i}: Point 1: theta(Length): {pair[0][0]:.2f} rad ({round(np.degrees(pair[0][0]), 2)}°), phi(Latitude): {pair[0][1]:.2f} rad ({round(np.degrees(pair[0][1]), 2)}°)  "
        #     f"|   Point 2: theta(Length): {pair[1][0]:.2f} rad ({round(np.degrees(pair[1][0]), 2)}°), phi(Latitude): {pair[1][1]:.2f} rad ({round(np.degrees(pair[1][1]), 2)}°)")
