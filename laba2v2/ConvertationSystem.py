...
def PolarToCartesian(r, theta, Num):
    ...
    matrix2_2D[Num][0] = x
    matrix2_2D[Num][1] = y
    print(f"Point {Num+1}: x: {matrix2_2D[Num][0]}, y: {matrix2_2D[Num][1]}")

def CartesianToPolar(x, y, Num):
    r = round(np.sqrt(x ** 2 + y ** 2), 2)
    theta = round(np.arctan2(y, x), 2)
    print(f"Point {Num+1}: r: {r}, theta: {theta} rad ({round(np.degrees(theta),2)}°)")
...
def SphericalToCartesian(r, theta, phi, Num):
    ...
    print(f"Point {Num+1}: x: {matrix2_3D[Num][0]}, y: {matrix2_3D[Num][1]}, z: {matrix2_3D[Num][2]}")
...
def CartesianToSpherical(x, y, z, Num):
    ...
    #print(r, theta, phi)
    print(f"Point {Num+1}: r: {r}, theta: {theta} rad ({round(np.degrees(theta),2)}°), phi: {phi} rad ({round(np.degrees(phi),2)}°)")

def print_2d_array(arr):
    for i, row in enumerate(arr):
        print(f"Point {i + 1}: r: {row[0]:.2f}, theta: {row[1]:.2f} rad ({round(np.degrees(row[1]),2)}°)")

def print_3d_array(arr):
    for i, row in enumerate(arr):
        print(f"Point {i + 1}: r: {row[0]:.2f}, theta: {row[1]:.2f} rad ({round(np.degrees(row[1]),2)}°), phi: {row[2]:.2f} rad ({round(np.degrees(row[2]),2)}°)")
